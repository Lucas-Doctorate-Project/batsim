...
enum class IPMessageType
{
    ...
    ,SCHED_TELL_ME_CARBON_FOOTPRINT   //!< Scheduler -> Server. The scheduler requests the platform's accumulated operational carbon footprint. The existing request name is retained for compatibility.
    ,SCHED_TELL_ME_WATER_FOOTPRINT   //!< Scheduler -> Server. The scheduler requests the platform's accumulated on-site plus off-site operational water footprint. The existing request name is retained for compatibility.
